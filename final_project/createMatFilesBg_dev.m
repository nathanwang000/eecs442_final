function [] = createMatFilesBg_dev(data, config)
% develop version, no parallel
totalIter = 1;
...
for iter=1:totalIter
    ...
    for iset = 1:length(imageSets)
        ...
        dataset = currentData;
        make_dir(outputFile);
        save(outputFile, 'dataset',  'integralData', 'imageInfo', '-v7.3'); % the last is just a format string that I don't understand
    end
end
