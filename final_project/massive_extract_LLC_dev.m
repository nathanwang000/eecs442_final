function massive_extract_LLC()
    % in this version, I only extract LLC for action(1)
    if 1
        global sched
	...
	config.dictionary.data = load([config.outputFolder '/dictionary_' 'board_airplane_Classification' num2str(dictionarySize) '.mat']); 
        % load input .mat file
        data = load(savefile);
        tic
