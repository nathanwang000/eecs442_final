...
if exist([config.outputFolder '/' data.name '/dictionary_' num2str(dictionarySize) '_caffe.mat'], 'file')
  fprintf('dictionary file found, using cached dictionary\n');
  dictionary = load([config.outputFolder '/' data.name '/dictionary_' num2str(dictionarySize) '_caffe.mat']);
  return;
end
...
for i=1:numImages
  ...
  inputFile = fullfile(config.outputFolder, data.name, 'caffe', currImage.folder, [currImage.filename(1:end-4) config.flipSuffix{flipIdx(i)} '.mat']);
...
save([config.outputFolder '/' data.name '/dictionary_' num2str(dictionarySize) '_caffe.mat'], 'dictionary');
